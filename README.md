...
The whole system runs **fully offline and deterministically** by default — the LLM boundary is a single scripted seam, so the demo and all **2,700+ tests** run with no network and no API key. Point it at a live model (Anthropic / OpenAI / Bedrock / Ollama) with a single flag.
...
- **Production-grade rigor on a portfolio codebase** — **2,700+ passing tests** (green in CI on Python 3.12 and 3.13), fully type-hinted (ships a PEP 561 `py.typed` marker), 16 context collectors, 15 CLI verbs, deterministic and offline end to end.
...
| 5 | A gate you armed tripped on a **finding** — the command itself succeeded and printed its normal output, but a `--fail-on-kind` gate matched at least one reported signal (`pla signals --workspace . --fail-on-kind secret_file`). Not a fault (`1`) and not an empty result (`2`): this is the channel a pre-commit hook or a CI step branches on. The matched kinds and counts are named on one `gate: ...` line on stderr; stdout is unchanged. |
