...
The whole system runs **fully offline and deterministically** by default — the LLM boundary is a single scripted seam, so the demo and all **6,000+ tests** run with no network and no API key. Point it at a live model (Anthropic / OpenAI / Bedrock / Ollama) with a single flag.
...
- **Production-grade rigor on a portfolio codebase** — **6,000+ passing tests** (green in CI on Python 3.12 and 3.13), fully type-hinted (ships a PEP 561 `py.typed` marker), 17 context collectors, 17 CLI verbs, deterministic and offline end to end.
...
- **[DIRECTIONS.md](DIRECTIONS.md)** — the scouting log, regenerated by the harness from
  the scouts' own notes: for each iteration that ran a scout pair it records the `lenses:` in
  play, every candidate considered, the `winner:` the PM picked and the `ship:` outcome.
  Coverage starts at `iter-92` and skips iterations that ran no scout, so it is the reasoning
  trail behind most of the commit history rather than all of it.
...
| `runs`    | List past dispatched runs under the state dir (`--status STATUS` narrows to runs of one status and composes with `--json`; `--json` for a JSON array). `--summary` swaps the listing for one aggregate of the same selection: run count, per-status histogram, and summed iterations/artifacts/retries/parse_errors (two human lines, or one `{runs, by_status, iterations, artifacts, retries, parse_errors}` object under `--json`; refused with `--prune`, exit 2). `--prune` turns the same selection into the product's retention operation: it reports the run dirs it would delete and **deletes nothing unless `--yes` is also given** (dry run is the default, exit 0 either way), selects with the *listing's own* `--status` filter so "what will be deleted" is answerable by a read-only command, is contained to direct `run-*` children of the state dir (a nested `run-*`, a plain file named `run-*`, and any other child are never touched), refuses a `run-*` **symlink** on one `refused:` stderr line rather than following it, and under `--json` emits one `{dry_run, status, selected, refused, deleted}` object.|
...
| 5 | A gate you armed tripped on a **finding** — the command itself succeeded and printed its normal output, but a gate you armed refused the result: a `--fail-on-kind` gate matched at least one reported signal (`pla signals --workspace . --fail-on-kind secret_file`), a `--fail-over N` budget saw **more** reported signals than `N` (`pla signals --workspace . --fail-over 30`), `verify --fail-on-unresolved` could not resolve a cited source against the snapshot (`pla verify --slate slate.json --snapshot snap.json --fail-on-unresolved`), or `diff --fail-on-change` saw at least one added, removed or changed goal (`pla diff --dir .pla_runs/stream --fail-on-change`). Not a fault (`1`) and not an empty result (`2`): this is the channel a pre-commit hook or a CI step branches on. Whichever gate tripped names itself on one `gate: ...` line on stderr; stdout is unchanged. |
...
code change. Seven settings also have a direct CLI flag (`--provider`,
`--scripted-responses`, `--state-dir`, `--workspace`, `--max-iterations`,
`--max-llm-calls`, and `--max-seconds`) and can be set either way;
...
| `PLA_MAX_SECONDS` | `--max-seconds` | *(none)* | Wall-clock ceiling in seconds for one dispatched loop invocation; checked before each PLAN, so the run stops BUDGET_EXHAUSTED after the iteration in flight. Unset = no time ceiling. Not persisted to `meta.json`, so a `resume` gets a fresh ceiling for its own leg. |
