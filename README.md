...
API key, no config file and no network** -- twelve of the seventeen verbs never
construct an LLM client at all. **Seven** of those twelve -- `collectors`,
`config`, `policy`, `providers`, `runs`, `signals` and `tools` -- need nothing
but the checkout itself. The other **five** are inspectors of a run that
already happened, so they are not standalone. `diff` and `explain` read a slate
file, `trace` reads a run directory's checkpoint, `trend` reads a whole
`watch --out-dir` stream directory of slates, and `verify` reads a slate
...
| `diff`    | Compare two saved slates and classify goals as added/removed/changed/unchanged (`--old A.json --new B.json`, or `--dir DIR` to diff the two newest `slate-<NNN>.json` ticks in a `watch --out-dir` stream directory — mutually exclusive with `--old`/`--new`; fewer than two ticks is a usage error (exit 2) that also prints a paste-ready `hint: pla watch --out-dir <that dir> ...` line naming the one command that produces the stream, so the refusal says what would satisfy it; `--json` for a JSON object; matched by normalized title; read-only, LLM-free).|
| `trend`   | Report which goals PERSIST across a `watch --out-dir` slate stream: per goal title, how many ticks it appears in plus the first and last tick index it was seen at, ranked by persistence (`--dir DIR` required; ONE slate is enough — unlike `diff --dir`, which needs two, because a persistence count is well-defined at N=1 — while zero stream slates is a usage error (exit 2) carrying the same paste-ready `hint: pla watch --out-dir ...` line `diff --dir` prints; entries that are not `slate-<NNN>.json` stream files are ignored; matched across ticks by normalized title, and a title repeated inside one slate counts once for that tick; `--json` for a `{total_ticks, goals[{title, score, ticks, first_seen, last_seen}]}` object; read-only, LLM-free, and it never gates, so it builds no settings).|
| `policy`  | Print the standing autonomy contract: the four ordered gate rules, the auto-dispatch threshold, and every category tagged sensitive/auto-eligible (`--json` for a JSON object; read-only, LLM-free, no workspace). `--check-goal '<CandidateGoal JSON>'` asks the same gate about ONE goal *you* name and prints that goal's decision audit instead of the catalog — still no slate, no LLM and no workspace — rendered by the same helpers `explain` uses, so it is byte-identical to `explain --goal-id` for the same goal; a supplied `score` is ignored (it is a computed field, so auto-dispatch cannot be forged), and a malformed or schema-invalid literal exits **1** on one `error:` line.|
...
names. Stdout becomes exactly one JSON object with seven always-present keys ---
...
(`{id, title}`, or `null` when the slate has no auto-dispatchable goal),
`dispatched`, and `deferred` (`[{id, title}]`, the goals the gate ALREADY
auto-approved that this single-goal invocation did not reach --- the machine twin
of the printed "auto-approved but NOT run" block, in the same ranked order, and
`[]` when the slate holds fewer than two auto-approved goals, so an unattended
supervisor can see the one class it may dispatch without a human) --- and
everything the default run prints to stdout goes to
...
`examples/check_autonomy.py` is the second committed consumer, and it grades the claim this
project leads with: it exits **1** when `explain --json`'s audit auto-dispatches a goal in a
sensitive category (approval is mandatory there at ANY score), auto-dispatches one below its
own `auto_dispatch_threshold`, or is too thin to exercise either rule. The sensitive set is
imported from `Settings`, never spelled, so widening it re-grades the demo with no edit here:
`python examples/check_autonomy.py < .pla_runs/explain.json`, fed by `make demo`.
...
| `meta.json` — in the run dir | once, as the run dir is created | Four keys, `{workspace_root, artifacts_dir, max_iterations, max_llm_calls}`. The workspace path is the one a later `resume` can get nowhere else: the checkpoint records the artifacts dir, but not the workspace the run was scanned from. The two budget keys are the run's *effective* L1 bound, so a `resume` — which declares no budget flag of its own — continues under the bound its `run` set instead of the built-in 8/24 default (precedence: `PLA_MAX_*` env > the recorded value > the default; an absent or `null` key keeps an older run dir resumable). |
...
failure. The machine-readable contract ships on the CLI itself: `pla --help`
ends with an `exit codes:` block covering all six codes, rendered from the
`_EXIT_CODES` literal in the source, so it is still there under `python -OO`.
The docstring of `proactive_loop.cli.main` and the table below are prose
derivations of that same block — easier to read, but `-OO` strips docstrings,
which empties the docstring in exactly the lean, scripted invocation most
likely to branch on an exit code.
...
code change. Six settings also have a direct CLI flag (`--provider`,
`--scripted-responses`, `--state-dir`, `--workspace`, `--max-iterations`,
and `--max-llm-calls`) and can be set either way;
the remaining eight are environment-only. **Precedence: an explicit CLI flag
...
| `PLA_MAX_ITERATIONS` | `--max-iterations` | `8` | Maximum PLAN/ACT/CHECK iterations for a single dispatched goal loop. |
| `PLA_MAX_LLM_CALLS` | `--max-llm-calls` | `24` | Hard cap on total LLM calls per session, a backstop budget across the whole run. |
