...
The whole system runs **fully offline and deterministically** by default — the LLM boundary is a single scripted seam, so the demo and all **5,500+ tests** run with no network and no API key. Point it at a live model (Anthropic / OpenAI / Bedrock / Ollama) with a single flag.
...
- **Production-grade rigor on a portfolio codebase** — **5,500+ passing tests** (green in CI on Python 3.12 and 3.13), fully type-hinted (ships a PEP 561 `py.typed` marker), 17 context collectors, 17 CLI verbs, deterministic and offline end to end.
