...
- **Production-grade rigor on a portfolio codebase** — **3,800+ passing tests** (green in CI on Python 3.12 and 3.13), fully type-hinted (ships a PEP 561 `py.typed` marker), 17 context collectors, 16 CLI verbs, deterministic and offline end to end.
...
| 5 | A gate you armed tripped on a **finding** — the command itself succeeded and printed its normal output, but a gate you armed refused the result: either a `--fail-on-kind` gate matched at least one reported signal (`pla signals --workspace . --fail-on-kind secret_file`), or a `--fail-over N` budget saw **more** reported signals than `N` (`pla signals --workspace . --fail-over 30`). Not a fault (`1`) and not an empty result (`2`): this is the channel a pre-commit hook or a CI step branches on. Whichever gate tripped names itself on one `gate: ...` line on stderr; stdout is unchanged. |
