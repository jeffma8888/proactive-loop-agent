...
- Graceful degradation (registry-driven): EVERY collector in all_collectors()
  ...
  hostile undecodable-content roots -- proving the SPEC §4.1 invariant for all 14
  collectors and auto-covering any future one with zero test edits.
- all_collectors(): the registry exposes EXACTLY the 14 documented collector types.
"""
...
# The 14 collectors documented in SPEC §4.1. Asserting the full name-SET (not a
...
class TestAllCollectors:
    def test_registry_covers_all_collector_types(self) -> None:
        """The registry must expose EXACTLY the 14 documented collectors (SPEC §4.1).
...
class TestGracefulDegradation:
    ...
    all_collectors()), so all 14 current collectors -- and any future one -- are
