...
factory iter 158, and 3,800+ -> 4,300+ at factory iter 180, when the live suite reached
4,308).

Coverage (numbered to match the iteration spec's Expected Behaviors):

1. The published floor is ``4,300+`` in BOTH intro sentences and the string
   ``3,800`` is gone from ``README.md`` entirely.
...
PUBLISHED_FLOOR = 4300
STALE_FLOOR_TOKEN = "3,800"
...
def test_readme_intro_publishes_the_bumped_floor_in_both_sentences() -> None:
    ...
    assert "**4,300+ tests**" in intro
    assert "**4,300+ passing tests**" in intro


def test_the_stale_floor_token_is_gone_from_the_readme() -> None:
    """``3,800`` occurred exactly twice in the repo, both above the marker."""
...
def test_the_bump_is_exactly_two_digit_tokens_while_head_is_pre_bump() -> None:
    ...
    if STALE_FLOOR_TOKEN not in head_intro:
        ...
        return
    assert intro.replace("4,300", STALE_FLOOR_TOKEN) == head_intro
...
def test_a_fabricated_floor_is_rejected_and_both_numbers_are_named() -> None:
    fabricated = _real_intro().replace("4,300+", "9,000+")
...
def test_a_true_but_stale_floor_is_rejected() -> None:
    stale = _real_intro().replace("4,300+", "1,000+")
...
def test_an_exact_count_is_still_rejected() -> None:
    exact = _real_intro().replace("4,300+", "4,357")
