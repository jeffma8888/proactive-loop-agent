...
engineer/reviewer notes were read, and no ``git diff`` was consulted.** The twelve
...
# ==========================================================================
# Behavior 1 --- `pla tools` exits 0 and prints a human catalog naming all 12
# canonical tools (each at least once).
# ==========================================================================


def test_b01_tools_exit0_lists_all_twelve(capsys):
...
# name-ascending and has exactly 12 elements.
# ==========================================================================
...
def test_b06_json_tools_elements_shape_order_count(capsys):
    obj = _tools_json(capsys)
    tools = obj["tools"]
    assert len(tools) == 12, f"tools array must have exactly 12 elements; got {len(tools)}"
    for t in tools:
    ...
    assert set(names) == CANONICAL_TOOLS, (
        f"the 12 names must be exactly the canonical set; got {sorted(names)}"
    )
...
def test_b09_tool_names_public_and_all_dispatchable(tmp_path):
    ...
    assert set(names) == CANONICAL_TOOLS, (
        f"tool_names() must return exactly the canonical 12 names; got {names}"
    )
    assert len(names) == 12, f"tool_names() must return 12 names; got {len(names)}"
