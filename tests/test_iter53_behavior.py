...
#                returns 13 collectors, one of them named "git_stash"; the class is
#                exported from proactive_loop.collectors; and the new kind flows
#                out through the existing `pla signals` inspector.
# ===========================================================================


def test_b11_registry_has_thirteen_with_git_stash(capsys) -> None:
    collectors = all_collectors()

    assert len(collectors) == 13, f"registry must now list 13 collectors; got {len(collectors)}"
