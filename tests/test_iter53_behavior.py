...
# ===========================================================================
# Behavior 11 -- Registry + end-to-end CLI integration. all_collectors() now
#                returns 12 collectors, one named "git_stash"; the class is
#                exported from proactive_loop.collectors; and the new kind flows
#                out through the existing `pla signals` inspector.
# ===========================================================================


def test_b11_registry_has_twelve_with_git_stash(capsys) -> None:
    collectors = all_collectors()

    assert len(collectors) == 12, f"registry must now list 12 collectors; got {len(collectors)}"
