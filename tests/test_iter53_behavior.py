...
#                returns 14 collectors, one of them named "git_stash"; the class is
#                exported from proactive_loop.collectors; and the new kind flows
#                out through the existing `pla signals` inspector.
# ===========================================================================


def test_b11_registry_has_fourteen_with_git_stash(capsys) -> None:
    collectors = all_collectors()

    assert len(collectors) == 14, f"registry must now list 14 collectors; got {len(collectors)}"
