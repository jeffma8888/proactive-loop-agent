...
def test_b09_the_floor_verdict_the_oracle_relies_on_still_bites(
    ...
    """
    intro = guard._intro()
    assert guard.suite_size_problems(intro, 5600 + guard.SUITE_SIZE_SLACK) != [], (
        "a floor a full slack behind the live count must be reported as stale"
    )
    assert guard.suite_size_problems(intro, 5600 + 1) == [], (
...
def test_b10_the_published_floor_constants_are_the_head_values() -> None:
    ...
    assert "PUBLISHED_FLOOR = 5600" in iter143, "the published floor moved unexpectedly"
    assert 'STALE_FLOOR_TOKEN = "5,500"' in iter143, "the stale-floor token moved unexpectedly"
...
def test_b10_the_readme_still_publishes_the_same_floor_claim() -> None:
    ...
    assert "5,600" in claim.group(0), claim.group(0)
    assert "5,500" not in intro, "the stale floor token reappeared in the README"
