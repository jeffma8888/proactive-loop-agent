...
was read, and no ``git diff`` was consulted.** The eleven canonical tool names and
...
# ===========================================================================
# EB1 --- Registered and dispatchable (11 names incl. tail_file)
# ===========================================================================


def test_eb01_registered_and_dispatchable(tmp_path: Path) -> None:
    names = ToolRegistry.tool_names()
    # Exactly 11 names, including tail_file.
    assert len(names) == 11, f"tool_names() must return 11 names; got {len(names)}: {names}"
...
def test_eb13_pla_tools_json_tail_file_object(capsys) -> None:
    ...
    # 11 tools total in the --json array.
    assert len(tools) == 11, f"--json tools array must have 11 elements; got {len(tools)}"
...
def test_eb13_pla_tools_human_lists_all_eleven(capsys) -> None:
    rc = main(["tools"])
    out = capsys.readouterr().out
    assert rc == 0, "`pla tools` must exit 0"
