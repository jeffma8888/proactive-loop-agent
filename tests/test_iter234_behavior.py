...
#: `diff` its exit-5 gate (`--fail-on-change`); 40 since foundry iter 321 gave `run`
#: the wall-clock ceiling (`--max-seconds`). A live flag the contract does not
...
def test_b7_readme_carved_out_numbers_still_agree_with_the_live_registries() -> None:
    ...
    assert "6,000+" in readme, "the README tests floor is no longer '6,000+'; behavior 7 expects it unchanged"
