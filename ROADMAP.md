...
| 3 | `pla explain --slate --goal-id` — print the score arithmetic + which gate rule fired + sources, making the autonomy contract auditable | CLI/L2 | High | Low | Scout A C1 (iter-02) | **SELECTED — iter-06** (blocker #2 shipped iter-02, so now unblocked; completes the scan → runs → explain legibility arc; additive cli.py-only, exposes the autonomy gate) |
