...
| 5 | `DependencyCollector` — new L2 signal reading project manifests (`pyproject.toml`/`package.json`/…) so the scout proposes stack-appropriate goals | L2 | High | Low | Scout A C1 (iter-01) | **SELECTED — iter-09** (highest-value unshipped net-new capability; L2 perception untouched since `e5694d9`; its sole deferral reason — observability-arc completion — was spent by iter-08; new `kind="dependency"` flows into synthesis via `by_kind()` with zero synthesizer change. Picked over the strong alt #4 CI+badge, which exposes no black-box behavior for the tester stage and whose first public run the loop cannot observe within an iteration — see iter-09 pm.md) |
