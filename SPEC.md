...
`create_client(settings)` switches on `settings.provider` over the seven names in
`VALID_PROVIDERS`, importing each optional SDK lazily inside its own branch so the default
`scripted` path stays offline and dependency-free. Settled contract prose: see
[SPEC_ARCHIVE.md](SPEC_ARCHIVE.md).
...
```
  ...
  **byte-identical** to `read_file` (no trailer, original terminators preserved via
  ...
  mirroring iter-13's `search_files`); `tail_file(path, max_lines=40)` → the LAST `max_lines` lines of a file — the mirror of `head_file`, a bounded bottom-of-file **peek** so a goal can inspect the END of a grown artifact / log / config (the block just `append_file`d, the tail where errors surface) without the unbounded `read_file`. SAME guards and SAME `artifacts_dir`-first / `workspace_root`-second precedence as `head_file`/`read_file`/`stat_file` (so `tail_file(x)` reads the same copy); for a file with `<= max_lines` lines the return is **byte-identical** to `read_file` (no trailer). Its ONE deliberate difference from `head_file` is the truncation shape: the trailer is a **LEADING** line `... (showing last {max_lines} of {total} lines)` followed by the file's last `max_lines` lines — the exact opposite of `head_file`'s **TRAILING** note (so the actual tail lines sit last, closest to the model's next reasoning step; `head_file` KEEPS its trailing note). Same `max_lines` rules (int or integer-valued string, non-positive/non-integer → `error: tail_file 'max_lines' must be a positive integer`, path-safety checked BEFORE `max_lines`), same not-found/read-only/never-raise behavior, completing the bounded-observation family as find / list / grep / describe / PEEK-top(head) / PEEK-bottom(tail) / read (additive tool added in iter-54 — existing tool contracts unchanged, so **no version bump**, mirroring iter-13's `search_files`); `read_lines(path, start, end)` → the 1-based **inclusive** line range `[start, end]` of a file — a FREE-anchor **interior window**, the fourth and final reader anchor completing the reader family as full (`read_file`) / top (`head_file`) / bottom (`tail_file`) / **window** (`read_lines`). It is the ONLY reader that consumes `search_files`' 1-based line numbers: to read the context AROUND a grep hit at line 250, `read_lines(path, 240, 260)` returns exactly that window without pulling the whole file through the unbounded `read_file`. Resolves `artifacts_dir` FIRST then `workspace_root` — the SAME precedence as `read_file`/`head_file`/`tail_file`/`stat_file` (so `read_lines(x)` reads the same copy). Its load-bearing distinction from `head_file`/`tail_file` (FIXED-anchor peeks that emit a truncation trailer) is that the return is a pure, byte-clean window `"".join(text.splitlines(keepends=True)[start-1:end])` with NO trailer and NO decoration, round-tripping the file's own line terminators verbatim (no added or stripped newline). `start`/`end` are positive integers (an int or an integer-valued string, mirroring `head_file`'s `max_lines`) validated `start` BEFORE `end` for a deterministic error order; a non-positive/non-integer/missing `start` → `error: read_lines 'start' must be a positive integer` (and `end` likewise → `error: read_lines 'end' must be a positive integer`), and `start > end` → `error: read_lines 'start' (<s>) must be <= 'end' (<e>)` (no reverse ranges). Path-safety errors (empty → `error: read_lines requires a non-empty 'path'`; `..`/absolute → the shared `_reject_unsafe` string) are reported BEFORE numeric validation, and nothing is read on any error. A `start` past EOF (`start > total`) → `error: read_lines 'start' (<s>) is past end of file (<total> lines): '<p>'` and nothing is read; an `end` beyond EOF is leniently CLAMPED like a Python slice (lines `start..last`, no error, no trailer). A safe path resolving to no file in either root → `error: file not found under artifacts or workspace: '<p>'` (the `read_file` not-found string). Read-only (never writes; `artifacts()` unaffected), and an undecodable (binary) file degrades to an `"error:"` via `execute()`'s never-raise wrapper (mirroring `read_file`'s decode behavior). Negative indexing, step/stride, and reverse ranges stay out of scope (1-based positive inclusive `[start, end]` with a lenient `end`-clamp only). (Additive tool added in iter-76 — existing tool contracts unchanged, so **no version bump**, mirroring iter-13's `search_files`); `remove_file(path)` → **deletes** a file under
  ...
  contracts unchanged, so **no version bump**, mirroring iter-13's `search_files`.); `replace_in_file(path, old, new)` → the write-side **EDIT** primitive: substitutes EVERY literal occurrence of `old` with `new` inside ONE existing artifact in a single deterministic `str.replace` call, completing the write-side mutation family create (`write_file`) / append (`append_file`) / **edit** / move (`move_file`) / delete (`remove_file`), and the natural CHECK-loop partner of `diff_files` (edit, then diff the artifact against a reference to verify WHAT CHANGED). Literal substring only — regex / anchored / nth-occurrence matching is out of scope — and it replaces ALL non-overlapping occurrences deterministically (no ambiguous-match failure mode), reporting an accurate count (`str.count(old)`, which matches `str.replace`'s non-overlapping count). Guard order is load-bearing for a mutation and mirrors `write_file`/`move_file`: the shared `_reject_unsafe` FIRST (empty → the generic `error: empty path is not allowed`, plus `..`/absolute), THEN a non-empty-`old` arg check reported AFTER path-safety (`error: replace_in_file 'old' must be non-empty`), THEN a resolved `_within` gate BEFORE any read/write (a symlink escaping the sandbox → `error: refusing to edit outside artifacts dir: <p>` and never edits through the link), THEN existence (`error: no such artifact: <p>`) and directory (`error: refusing to edit a directory: <p>`) checks. It resolves ONLY against `artifacts_dir` — never against, and never writing through to, the read-only `workspace_root` (a workspace-only path degrades to `no such artifact`) — reads with `read_text` and writes with plain `write_text` (mirroring `write_file`, NOT tmp+`os.replace`). When `old` is not present → `error: text not found in artifacts/<relpath>: <old!r>` and the file is untouched; on success it writes `content.replace(old, new)` and returns `replaced <count> occurrence(s) in artifacts/<relpath>`, recording the relpath in `artifacts()` when absent (an on-disk artifact not written this run is added, exactly once). An empty `new` (or an omitted `new` key) deletes the matched substring. Never raises — an unsafe/empty path, empty `old`, missing target, directory, or text-not-found degrades to an `"error: ..."` observation, and a binary/undecodable file surfaces as an `"error:"` via `execute()`'s never-raise wrapper (mirroring `read_file`'s decode behavior). Regex/pattern and a `count`/`nth` argument stay out of scope (the arg surface is exactly `(path, old, new)`). (Additive tool added in iter-66 — existing tool contracts unchanged, so **no version bump**, mirroring iter-13's `search_files`.) Unknown tool →
...
- `cli.py` (argparse, `main(argv=None) -> int`, console script `pla`, global `--version`):
  ...
  - `pla tools [--json]` — read-only, LLM-free, zero-input catalog of the L1 ACT
    ...
    `search_files`/`find_files`/`tail_file`/`read_lines`/`diff_files` → `read-only`; `move_file` → `move`; `remove_file`
    ...
    array of 14 `{name, access, description}` objects (exactly those three keys
  ...
  - `pla collectors [--json]` — read-only, LLM-free, zero-input catalog of the L2
    ...
    schema-leak discipline): `collectors` is a name-ascending array of 17
